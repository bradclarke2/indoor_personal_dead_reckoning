...
%% X-Y co-ordinate plotting
    ...
    plot(A)
     ...
     Velocity_stairs = 0.2; % average x velocity whilst travelling up stairs
   ...
   for n = 1 : length(weighted)
    ...
    if  weighted(n) > 20 && weighted(n) < 30
        ...
        B(n) = 25;
        x_velocity = Velocity * cos(theta(28*n)) ; %veloctiy of person in x m/s
        y_velocity = Velocity * sin (theta(28*n));
        
    elseif weighted(n) > 30 && weighted(n)<40
        
        B(n) = 35;
        x_velocity = Velocity_stairs * cos(theta(29*n));
        y_velocity = Velocity_stairs * sin (theta(29*n));
    ...
    y_distance(n+1) = y_distance(n) + y_velocity * time_step_characterised;
   end
   figure
   plot(axis_spacing,B)
   figure
